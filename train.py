...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples = 2):
    ...
    expected = []
    predicted = []
    console_width = 80 #size of the control window (just a default value)
    with torch.no_grad():
...
def get_ds(config):
    ...
    tokenizer_src = get_or_build_tokenizer(config, ds_raw, config['lang-src'])
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, config['lang-tgt'])
    ...
    train_ds = BilingualDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, config['lang-src'], config['lang-tgt'], config['seq_len'])
    val_ds = BilingualDataset(val_ds_raw, tokenizer_src, tokenizer_tgt, config['lang-src'], config['lang-tgt'], config['seq_len'])

    max_len_src = 0
    max_len_tgt = 0

    for item in ds_raw:
        src_ids = tokenizer_src.encode(item['translation'][config['lang-src']]).ids
        tgt_ids = tokenizer_tgt.encode(item['translation'][config['lang-tgt']]).ids
...
def train_model(config):
    ...
    train_dataloader, val_data_loader, tokenizer_src, tokenizer_tgt = get_ds(config)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
         ...
         for batch in batch_iterator:
             ...
             optimizer.zero_grad()
             global_step += 1
         run_validation(model, val_data_loader, tokenizer_src, tokenizer_tgt, config['seq_len'], device,
